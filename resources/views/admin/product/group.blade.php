    ...
    <div class="ml-64 py-12">
        <div class=" mx-auto sm:px-6 lg:px-8">
            <form class="grid grid-cols-12 gap-2" method="get" action="{{route('admin.product.list')}}">
                ...
                <a href="{{route('admin.product.add.page')}}"
                   class="text-white bg-blue-800 hover:bg-blue-900 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-800 dark:hover:bg-blue-700 dark:focus:ring-blue-700 dark:border-blue-700 col-span-2 text-center">
                    Tambah
                </a>
            </form>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="datatables-user">
                            ...
                            @foreach($groups as $group)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    @if($selectedGroup != 0)
                                        <td class="py-4 px-6">
                                            {!! QrCode::size(50)->generate($group->code) !!}
                                        </td>
                                    @endif
                                    <td class="py-4 px-6">
                                        Rp. {{number_format($group->buy_price, 2, ',', '.')}}
                                    </td>
                                    <td class="py-4 px-6">
                                        {{$group->total}}
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        @if($selectedGroup == 0)
                                            <a href="{{route('admin.product.group.detail.page', [$selectedGroup, $group->weight])}}">
                                        ...
                                        @else
                                            <a href="{{route('admin.product.group.detail.page', [$group->weight,$group->qr_group_id])}}">
                                                <svg class="w-4 h-4 m-2" fill="none" stroke="currentColor"
                                                     viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
